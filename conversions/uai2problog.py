...
class UAIParser(BNParser):
    ...
    def construct_cpt(self, func_num):
        if func_num >= len(self.func_vars) or self.func_vars[func_num] is None:
            logger.error(
                "Variables not defined for function {}".format(func_num), halt=True
            )
            sys.exit(1)
        ...
        try:
            ...
            logger.error(
                "Could not find function definition for {}".format(func_num), halt=True
            )
            sys.exit(1)
        else:
            idx = 0
    ...
    def parse_header(self):
        ...
        if token == "BAYES":
            ...
            self.directed = None
            logger.error(
                "Expected a BAYES or MARKOV network, found: "
                + "{} (line {})".format(token, self.reader.linenb),
                halt=True,
            )
            sys.exit(1)
        ...
        if len(self.dom_sizes) != self.num_vars:
            logger.error(
                "Expected {} domain sizes, ".format(self.num_vars)
                + "found {} (line {})".format(len(self.dom_sizes), self.reader.linenb),
                halt=True,
            )
            sys.exit(1)
        ...
        if self.directed and self.num_funcs != self.num_vars:
            logger.error(
                "For BAYES we expect one function for every variables but found: "
                + "{} (line {})".format(self.num_funcs, self.reader.linenb),
                halt=True,
            )
            sys.exit(1)
    ...
    def parse_functions(self):
        ...
        for num_func in range(self.num_funcs):
            ...
            if exp_num_values != num_values:
                logger.warning(
                    (
                        "% WARNING: Function {} says {} values but {} values "
                        + "are expected given the domain size "
                        + "for variable {}."
                    ).format(num_func, num_values, exp_num_values, num_func)
                )
            tokens = self.reader.get_tokens(num_values)
            values = [float(v) for v in tokens]
            if len(values) != num_values:
                logger.error(
                    "Expected {} values in function {}, ".format(num_values, num_func)
                    + "found {} (line {})".format(len(values), self.reader.linenb),
                    halt=True,
                )
                sys.exit(1)
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description="Translate Bayesian net in UAI08 format to ProbLog"
    )
    parser.add_argument(
