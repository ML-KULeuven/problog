...
class PGM(object):
    ...
    def split_topological(self, split_vars):
        ...
        factor_strata = self.factors_topological(
            keep_strata=True, delay_roots=[delay_roots]
        )
        ...
        for factor in self.factors.values():
            if strata[factor.rv] <= max_strata and any(
                (strata[cvar] > max_strata for cvar in children[factor.rv])
            ):
                nb_values = len(self.vars[factor.rv].values)
                pgm.add_var(self.vars[factor.rv].copy(name=rootvar(factor.rv)))
                pgm.add_factor(
                    Factor(pgm, rootvar(factor.rv), [], [1.0 / nb_values] * nb_values)
                )
            if strata[factor.rv] > max_strata and any(
                (strata[pvar] <= max_strata for pvar in factor.parents)
            ):
                parents = []
    ...
    def split(self, variables):
        ...
        for factor in self.factors.values():
            if factor.rv in variables:
                nb_values = len(self.vars[factor.rv].values)
                pgm.add_factor(
                    Factor(pgm, factor.rv, [], [1.0 / nb_values] * nb_values)
                )
    ...
    def to_hugin_net(self, include_layout=False):
        ...
        if include_layout:
            lines += ["net {", "  node_size = (50,50);", "}\n"]
        lines += ["%% Nodes\n"]
        lines += [cpd.to_huginnet_node(include_layout=include_layout) for cpd in cpds]
        lines += ["%% Potentials\n"]
        lines += [
            cpd.to_huginnet_potential(include_layout=include_layout) for cpd in cpds
        ]
        return "\n".join(lines)
    ...
    def to_xdsl(self):
        ...
        lines += [
            "  </nodes>",
            "  <extensions>",
            '    <genie version="1.0" app="ProbLog" name="{}" faultnameformat="nodestate">'.format(
                self.name
            ),
        ]
        lines += [cpd.to_xdsl_node() for cpd in cpds]
        lines += ["    </genie>", "  </extensions>", "</smile>"]
        return "\n".join(lines)
    ...
    def to_uai08(self):
        ...
        lines = ["BAYES", number_variables, " ".join(domain_sizes), number_functions]
    ...
    def to_problog(
        ...
        lines = [
            "%% ProbLog program: {}".format(self.name),
        ...
        if self.directed:
            lines += [
                factor.to_problog(
                    ...
                    ad_is_function=ad_is_function,
                )
                for factor in factors
            ]
        else:
            lines += [
                factor.to_problog_undirected(
                    self,
                    drop_zero=drop_zero,
                    use_neglit=use_neglit,
                    value_as_term=value_as_term,
    ...
    def to_graphviz(self):
        ...
        for cpd in self.factors.values():
            ...
            for p in cpd.parents:
                lines.append(
                    "  {} -> {}".format(
                        self.vars[cpd.rv].clean(p), self.vars[cpd.rv].clean()
                    )
                )
        lines += ["}"]
        return "\n".join(lines)
        ...
        if (force_boolean or detect_boolean) and len(self.values) == 2:
            for values in boolean_values:
                if (values[0] == self.values[0] and values[1] == self.values[1]) or (
                    type(self.values[0]) == str
                    and type(self.values[1]) == str
                    and values[0] == self.values[0].lower()
                    and values[1] == self.values[1].lower()
                ):
                    self.boolean_true = 1
                    break
                    ...
                    type(self.values[0]) == str
                    and type(self.values[1]) == str
                    and values[1] == self.values[0].lower()
                    and values[0] == self.values[1].lower()
                ):
                    self.boolean_true = 0
                    break
                ...
                if len(value) == len(self.values) - 1:
                    ...
                    if value_as_term:
                        ...
                        return (
                            ...
                            + "; ".join(
                                [
                                    self.clean() + "_" + self.clean(str(new_value))
                                    for new_value in value
                                ]
                            )
                            + ")"
...
class Factor(object):
    def __init__(self, pgm, rv, parents, table, name=None, *args, **kwargs):
        ...
        if isinstance(table, list) or isinstance(table, tuple):
            ...
            raise ValueError(
                "Unknown type (expected list, tuple or dict): {}".format(type(table))
            )
        ...
        while len(nodes) > 0:
            ...
            for parent_idx, parent in enumerate(self.parents):
                ...
                for bin_value, bin_labels in bins.items():
                    ...
                    ps = [
                        cnt / len(bin_labels) for cnt in label_cnt.values()
                    ]  # Pr(x_i)
            ...
            if igr_idx is None:
                ...
                for new_path, new_probs in node:
                    new_table[new_path] = new_probs
                continue
            if allow_disjunct and len(
                self.pgm.vars[self.parents[igr_idx]].values
            ) > max(2, len(c)):
    ...
    def to_huginnet_node(self, include_layout=False):
        ...
        lines = ["node {} {{".format(rv.clean()), '  label = "{}";'.format(self.rv)]
        if include_layout:
            lines += ["  position = (100,100);"]
        lines += [
            "  states = ({});".format(
                " ".join(['"{}"'.format(v) for v in sorted(rv.values)])
            ),
            "}\n",
        ]
        return "\n".join(lines)
    ...
    def to_huginnet_potential(self, include_layout=False):
        ...
        for k, probs in table:
            lines.append(
                "    "
                + " ".join([str(probs[value_idx[1]]) for value_idx in value_idxs])
                + " % "
                + " ".join([str(kk) for kk in k])
            )
        lines += ["  );", "}\n"]
        return "\n".join(lines)
        ...
        if len(self.parents) > 0:
            lines.append(
                "      <parents>{}</parents>".format(
                    " ".join([rv.clean(p) for p in self.parents])
                )
            )
        table = sorted(self.table.items())
        probs = " ".join(
            [str(probs[value_idx[1]]) for k, probs in table for value_idx in value_idxs]
        )
    ...
    def to_xmlbif_cpt(self):
        rv = self.pgm.vars[self.rv]
        lines = ["<DEFINITION>", "  <FOR>{}</FOR>".format(rv.name)]
    ...
    def to_xmlbif_node(self):
        rv = self.pgm.vars[self.rv]
        lines = ['<VARIABLE TYPE="nature">', "  <NAME>{}</NAME>".format(rv)]
    ...
    def to_uai08_preamble(self, cpds):
        ...
        variables = [str(rv_to_idx[rv]) for rv in self.parents] + [
            str(rv_to_idx[self.rv])
        ]
    ...
    def to_problog(
        ...
        for k, v in table:
            if (
                var.boolean_true is not None
                and drop_zero
                and v[var.boolean_true] == 0.0
                and not use_neglit
            ):
                continue
            head_problits = []
            if var.boolean_true is None:
                for idx, vv in enumerate(v):
                    if not (drop_zero and vv == 0.0):
                        head_problits.append(
                            (vv, var.to_problog_value(var.values[idx], value_as_term))
                        )
                ...
                if drop_zero and v[var.boolean_true] == 0.0 and use_neglit:
                    head_problits.append(
                        (
                            None,
                            var.to_problog_value(
                                var.values[1 - var.boolean_true], value_as_term
                            ),
                        )
                    )
                    ...
                    head_problits.append(
                        (
                            None,
                            var.to_problog_value(
                                var.values[var.boolean_true], value_as_term
                            ),
                        )
                    )
                else:
            ...
            for parent, parent_value in zip(self.parents, k):
                if parent_value is not None:
                    ...
                    body_lits.append(
                        parent_var.to_problog_value(parent_value, value_as_term)
                    )
            ...
            if ad_is_function:
                for head_cnt, (head_prob, head_lit) in enumerate(head_problits):
                    if len(body_lits) == 0:
                        ...
                        lines.append(
                            "({};1.0)::{} :- {}.".format(
                                head_prob, head_lit, ", ".join(body_lits)
                            )
                        )
            else:
                if len(body_lits) > 0:
        ...
        if ad_is_function and var.boolean_true is None:
            head_lits = [
                var.to_problog_value(value, value_as_term) for value in var.values
            ]
    ...
    def __str__(self):
        ...
        if self.rv is not None:
            ...
            if len(self.parents) > 0:
                rv += " | "
            return "Factor ({}{}) = {}\n{}\n".format(
                rv, parents, ", ".join([str(v) for v in var.values]), table
            )
...
class OrCPT(Factor):
    ...
    def __str__(self):
        ...
        return "OrCPT {} [{}]{}\n{}\n".format(
            self.rv, ",".join(map(str, rv.values)), parents, table
        )
