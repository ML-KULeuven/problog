...
import problog.engine_messages
...
class TreeDraw(QWidget):
    ...
    def drawTree(self, p):
        ...
        if coordnodes:
            ...
            for a, b in coordedges:
                ...
                p.drawLine(x1+self.NODE_WIDTH/2, y1+self.NODE_HEIGHT, x2+self.NODE_WIDTH/2, y2)
    ...
    def drawNode(self, p, coord, node):
        ...
        p.drawText(x, y, self.NODE_WIDTH, self.NODE_HEIGHT, Qt.AlignVCenter | Qt.AlignLeft, node['text'])
    ...
    def mousePressEvent(self, event):
        ...
        for i, c in enumerate(self.coordinates):
            if c[0] <= x <= c[0] + self.NODE_WIDTH and c[1] <= y <= c[1] + self.NODE_HEIGHT:
...
class MessageOrderInteractive(problog.engine_messages.MessageOrder1):

    def __init__(self, engine):
        problog.engine_messages.MessageOrder1.__init__(self, engine)
        self.cv = Condition()
        self.choice = None
    ...
    def append(self, message):
        ...
        if message[0] == 'e':
            print ('append', message[0:3], message[3].get('call'), message[3].get('context'), file=sysout)
    ...
    def stack2tree(self):
        ...
        for i, node in enumerate(self.engine.stack):
            if node is not None:
                ...
                nodes[name] = {'name': name, 'text': self.node2text(node), 'parent': parent, 'cycle': cc}
